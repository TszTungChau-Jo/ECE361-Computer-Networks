...
bool sock_receive(SocketObj * sock, charArray* message_string, double time_delay){ ////////////////////////////////////////////////////////////////////////

  // Set up a timeval struct for the timeout
  struct timeval timeout;
  timeout.tv_sec = time_delay;  // 5 seconds
  timeout.tv_usec = 0;
...
int main(int argc, char * argv[]){
  ...
  if(!hasConnection){
    struct timeval start_time, end_time;
      gettimeofday(&start_time, NULL);  // Record the start time
        sock_send(&sock,"ftp");
        sock_receive(&sock, &message_string,0.0005);
      gettimeofday(&end_time, NULL);  // Record the end time
      time_delay = (end_time.tv_sec - start_time.tv_sec) + (end_time.tv_usec - start_time.tv_usec) / 1000000.0;
  ...
  while (file_size > 0) {
    ...
    strcpy(message_string2.array, header);
    memcpy(message_string2.array + strlen(header), buffer, sizeof(buffer)); // copy buffer into message_string.array, at the location right after the string header
    message_string2.len=strlen(header)+bytes_read;
    ...
    while(1){
      sock_send_raw(&sock, &message_string2);

      if(sock_receive(&sock, &message_string, 1) && atoi(message_string.array)==frag_no){
      ...
      printf("deliver: did not got ack, resending %d\n\n",frag_no);
    }
